...
class GcodeWriter:
    ...
    def __init__(self, filename=None, scale=1, extruder=True, x_offset=20, y_offset=20, z_offset=0):
    ...
    def convert_print(self, total_path):

        with open("./config/header.txt") as f:
        ...
        for idx, path in enumerate(total_path):
            ...
            # undo retraction
            output += "G1 E1.40000 F1500.00000;\n"

            # pen down
            output += "G01 Z" + str(idx) + ";\n"
            
            p0 = path[0]

            # trace the path
            for p1 in path[1:]:
                output += self.command_print(p0, p1, 0.031617)
                p0 = p1
                
            # retraction
            output += "G1 E-1.40000 F1500.00000;\n"

            # pen up
            output += "G01 Z" + str(idx) + ";\n"

        with open("./config/footer.txt", "r") as f:
            output += "".join(f.readlines())
        f.close()
